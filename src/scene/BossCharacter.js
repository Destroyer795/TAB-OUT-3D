...
export class BossCharacter {
    ...
    _build() {
        ...
        // Torso (Suit Jacket)
        // Tall torso spanning from Y=1.2 to 2.3
        const torsoGeo = new THREE.CylinderGeometry(0.35, 0.28, 1.1, 16);
        const torso = new THREE.Mesh(torsoGeo, suitMat);
        torso.scale.z = 0.6; // Flatten cylinder front-to-back
        torso.position.y = 1.75; // Center of torso
        this.modelGroup.add(torso);

        // Shirt (plane on front of upper torso)
        const shirtGeo = new THREE.PlaneGeometry(0.2, 0.6);
        const shirt = new THREE.Mesh(shirtGeo, shirtMat);
        shirt.position.set(0, 1.95, 0.18);
        this.modelGroup.add(shirt);

        // Tie (on front of shirt)
        const tieGeo = new THREE.PlaneGeometry(0.05, 0.5);
        const tie = new THREE.Mesh(tieGeo, tieMat);
        tie.position.set(0, 1.85, 0.19);
        this.modelGroup.add(tie);

        // Collar Details
        const collarGeo = new THREE.PlaneGeometry(0.08, 0.05);
        const collarL = new THREE.Mesh(collarGeo, shirtMat);
        collarL.position.set(-0.04, 2.22, 0.19);
        collarL.rotation.z = -0.3;
        this.modelGroup.add(collarL);

        const collarR = new THREE.Mesh(collarGeo, shirtMat);
        collarR.position.set(0.04, 2.22, 0.19);
        collarR.rotation.z = 0.3;
        this.modelGroup.add(collarR);

        // Head (positioned high at Y=2.62)
        ...
        const briefcaseGeo = new THREE.BoxGeometry(0.12, 0.28, 0.38);
        const briefcase = new THREE.Mesh(briefcaseGeo, briefcaseMat);
        briefcase.position.set(0.05, -1.25, 0.05);
        this._rightArm.add(briefcase);

        const handleGeo = new THREE.BoxGeometry(0.02, 0.07, 0.12);
        const briefcaseHandle = new THREE.Mesh(handleGeo, briefcaseMat);
        briefcaseHandle.position.set(0.05, -1.08, 0.05);
        this._rightArm.add(briefcaseHandle);

        // Leg Pivots (located at the hips Y=1.2)
        this._leftLeg = new THREE.Group();
        this._leftLeg.position.set(-0.18, 1.2, 0);
        this.modelGroup.add(this._leftLeg);

        this._rightLeg = new THREE.Group();
        this._rightLeg.position.set(0.18, 1.2, 0);
        this.modelGroup.add(this._rightLeg);

        // Leg Meshes
        const legGeo = new THREE.CylinderGeometry(0.14, 0.11, 1.1, 16);
        ...
        legMeshR.position.y = -0.55;
        this._rightLeg.add(legMeshR);

        // Shoes
        const shoeGeo = new THREE.BoxGeometry(0.22, 0.15, 0.35);
        const shoeL = new THREE.Mesh(shoeGeo, shoeMat);
        shoeL.position.set(0, -1.15, 0.03);
        this._leftLeg.add(shoeL);
        const shoeR = new THREE.Mesh(shoeGeo, shoeMat);
        shoeR.position.set(0, -1.15, 0.03);
        this._rightLeg.add(shoeR);

        // Front Spotlight to illuminate the boss's face and upper torso
        this.frontLight = new THREE.PointLight(0xffffff, 2.2, 6);
        this.frontLight.position.set(0, 2.4, 1.3);
    ...
    updateWalk(progress) {
        ...
        // Subtle body bobbing
        this.modelGroup.position.y = Math.abs(Math.sin(progress * Math.PI * 6)) * 0.06;
    }
